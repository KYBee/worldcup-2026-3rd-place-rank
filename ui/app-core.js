...
export async function loadTournamentData() {
  ...
  scheduleData.fixtures.forEach((fixture) => {
    ...
    if (!homeTeamId || !awayTeamId) {
      fixtureWarnings.push(`${fixture.matchId}: ${fixture.homeTeam} vs ${fixture.awayTeam}`);
      return;
    }
    fixtures.push({
...
export function getLockedMatchIds(data) {
  return new Set(Object.keys(readOfficialScores(data)));
}

export function buildEffectiveScores(data, scores) {
  return {
    ...(scores || {}),
    ...readOfficialScores(data),
  };
}
...
export function buildScoresWithDefaultDraw(data, scores) {
  ...
  const effectiveScores = buildEffectiveScores(data, scores);

  data.fixtures.forEach((fixture) => {
    const existing = effectiveScores[fixture.matchId];
